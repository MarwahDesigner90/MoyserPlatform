...
def add_feedback_view(request, companion_id):
    ...
    print(companion.id)
    print(companion.companion.id)
    # Get the user's latest booking for this companion
    user_booking = Booking.objects.filter(user=request.user, companion=companion).last()
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            feedback.disability_user = DisabilityUser.objects.get(user=request.user)  
